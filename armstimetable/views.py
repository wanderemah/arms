...
from .models import Lecturer, Event, Comment, Responses, Signup, Student
from armsfinal.forms import Userform, Add, Queries, Response, SignForm
...
def topic_detail2(request, topic):
    subject = Event.objects.get(pk=topic)
    return render(request, 'armstimetable/topic.html', {'topic': subject})
...
def tutor_detail2(request, tutor):
    person = Lecturer.objects.get(name=tutor)
    return render(request, 'armstimetable/tutor.html', {'person': person})


# def student_detail(request, student):
#     person = Student.objects.get(name=student)
#     return render(request, 'itinerary/tutor.html', {'person': person})
        ...
        if user_form.is_valid() and profile_form.is_valid():
            ...
            if user is not None:
                if user.is_active and user.is_staff:
                    ...
                    login(request, user)
                    return redirect('timetable:docket2')
